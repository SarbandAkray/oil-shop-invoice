...
<head>
    ...
    <title>{{ __('invoice.edit') }} — {{ $invoice->number }}</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .company-name-text { font-size: 1.05rem; font-weight: 700; color: #222; margin-top: 5px; }
        .header-right { text-align: right; font-size: 0.85rem; color: #444; line-height: 1.75; }
        ...
        /* Info Grid */
        .info-grid { display: grid; grid-template-columns: 1fr 1fr; border: 2px solid #222; margin-bottom: 0; direction: ltr; }
        ...
        .info-block-title { font-weight: 700; font-size: 0.9rem; margin-bottom: 10px; padding-bottom: 5px; border-bottom: 1px solid #ddd; color: #222; }
        .form-row { display: flex; flex-direction: column; margin-bottom: 9px; }
        .form-row:last-child { margin-bottom: 0; }
        .form-row label { font-size: 0.78rem; font-weight: 600; color: #555; margin-bottom: 3px; text-transform: uppercase; letter-spacing: 0.4px; }
        .form-row input, .form-row select { padding: 7px 10px; border: 1px solid #ccc; border-radius: 3px; font-size: 0.92rem; width: 100%; font-family: inherit; }
        ...
        /* Table */
        table { width: 100%; border-collapse: collapse; border: 2px solid #1565c0; border-top: none; }
        thead { border-top: 2px solid #1565c0; }
        th { background: #1565c0; color: #ffd600; padding: 10px 12px; font-size: 0.85rem; text-align: left; border: 2px solid #1565c0; font-weight: 700; }
        [dir="rtl"] th { text-align: right; }
        td { padding: 7px 10px; border: 2px solid #90bce8; vertical-align: middle; }
        td input { width: 100%; padding: 6px 8px; border: 1px solid #ddd; border-radius: 3px; font-size: 0.9rem; font-family: inherit; }
        td input:focus { outline: none; border-color: #1565c0; }

        /* Add item row */
        .add-item-row { text-align: center; padding: 8px; border: 2px solid #1565c0; border-top: none; }
        .btn-add { background: #1565c0; color: #fff; border: none; padding: 7px 20px; border-radius: 3px; cursor: pointer; font-size: 0.88rem; font-weight: 600; }
        .btn-add:hover { background: #1976d2; }
        .btn-delete { background: #dc2626; color: #fff; border: none; padding: 5px 11px; border-radius: 3px; cursor: pointer; font-size: 0.85rem; }
        .btn-delete:hover { background: #b91c1c; }

        /* Bottom row */
        .bottom-row { display: flex; border: 2px solid #222; border-top: none; }
        .notes-col { flex: 1; padding: 12px; border-right: 2px solid #222; }
        [dir="rtl"] .notes-col { border-right: none; border-left: 2px solid #222; }
        .notes-col label { font-size: 0.8rem; font-weight: 600; color: #555; display: block; margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.4px; }
        .notes-col textarea { width: 100%; border: 1px solid #ddd; border-radius: 3px; padding: 7px; font-size: 0.9rem; resize: vertical; min-height: 90px; font-family: inherit; color: #333; }
        .notes-col textarea:focus { outline: none; border-color: #1565c0; }

        .totals-col { width: 290px; }
        .total-row { display: flex; justify-content: space-between; align-items: center; padding: 8px 14px; border-bottom: 1px solid #eee; }
        .total-row:last-child { border-bottom: none; }
        .total-label { font-size: 0.88rem; font-weight: 600; color: #444; }
        .total-value { font-size: 0.88rem; font-weight: 700; color: #222; }
        .total-input { width: 110px; padding: 5px 8px; border: 1px solid #ccc; border-radius: 3px; font-size: 0.88rem; text-align: right; font-family: inherit; }
        [dir="rtl"] .total-input { text-align: left; }
        .total-input:focus { outline: none; border-color: #1565c0; }
        .remaining-row { background: #f0f7ff; }

        /* Action buttons */
        .action-buttons { display: flex; justify-content: center; gap: 16px; margin-top: 20px; padding-top: 14px; border-top: 1px solid #ddd; }
        .btn-submit { background: #1565c0; color: #fff; border: none; padding: 10px 32px; border-radius: 5px; cursor: pointer; font-size: 0.95rem; font-weight: 700; }
        .btn-submit:hover { background: #1976d2; }
        .btn-cancel { background: #555; color: #fff; border: none; padding: 10px 24px; border-radius: 5px; cursor: pointer; font-size: 0.95rem; font-weight: 700; text-decoration: none; }
...
<body>
    ...
    <form method="POST" action="{{ route('invoice.update', $invoice->id) }}">
        ...
        <div class="invoice-wrapper">
            ...
            <div class="invoice-title-bar">{{ __('invoice.edit') }}: {{ $invoice->number }}</div>
                ...
                <div class="info-block">
                    <div class="form-row">
                        ...
                        <select id="currency" name="currency" onchange="updateCurrencySymbol()">
                            <option value="IQD" @selected($invoice->currency === 'IQD')>IQD (د.ع)</option>
                    ...
                    <div class="form-row">
                        ...
                        <input type="date" name="date" value="{{ $invoice->date->format('Y-m-d') }}" required>
                    </div>
                </div>
                <div class="info-block">
                    ...
                    <div class="form-row">
                        ...
                        <input type="text" name="customer_name" value="{{ $invoice->customer_name }}">
                    </div>
                    <div class="form-row">
                    ...
                    <div class="form-row">
                        ...
                        <input type="tel" name="phone" value="{{ $invoice->phone }}">
                    </div>
                </div>
            </div>
            ...
            <div class="bottom-row">
                <div class="notes-col">
                    ...
                    <textarea name="notes" placeholder="{{ __('invoice.notes') }}...">{{ $invoice->notes }}</textarea>
                </div>
                <div class="totals-col">
                    ...
                    <div class="total-row">
                        ...
                        <input class="total-input" type="number" id="tax" name="tax" min="0" step="0.01"
                               value="{{ $invoice->tax }}" onchange="recalculate()">
                    </div>
                    <div class="total-row">
                        <span class="total-label">{{ __('invoice.paid') }}</span>
                        <input class="total-input" type="number" id="paid" name="paid" min="0" step="0.01"
                               value="{{ $invoice->paid }}" onchange="recalculate()">
                    </div>
            ...
            <div class="action-buttons">
                <a href="{{ route('invoice.show', $invoice->id) }}" class="btn-cancel">← {{ __('invoice.go_home') }}</a>
                <button class="btn-submit" type="submit">💾 {{ __('invoice.print') }}</button>
            </div>
    ...
    <script>
        const existingItems = @json($invoice->items->map(fn($i) => [
            ...
            'unit_type'   => $i->unit_type   ?? 'single',
            'unit_detail' => $i->unit_detail ?? '',
        ]));
    </script>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            currencyEl.addEventListener('change', (e) => { currentCurrency = e.target.value; render(); });

            invoiceItems = existingItems.map(i => ({
                name: i.name, quantity: i.quantity, unit_price: i.unit_price,
                unit_type: i.unit_type || 'single', unit_detail: i.unit_detail || ''
            }));
            if (invoiceItems.length === 0) invoiceItems.push({ name: '', quantity: 1, unit_price: 0, unit_type: 'single', unit_detail: '' });
            render();
        });

        function sym() { return currentCurrency === 'IQD' ? 'د.ع ' : '$'; }
        function fmt(v) { v = parseFloat(v) || 0; return v % 1 !== 0 ? v.toFixed(2) : v.toLocaleString(); }
        ...
        function render() {
            ...
            invoiceItems.forEach((item, i) => {
                ...
                row.innerHTML = `
                    <td><input type="text" name="items[${i}][name]" value="${escHtml(item.name)}" onchange="invoiceItems[${i}].name=this.value" required></td>
                    ...
                    <td><input type="number" min="1" name="items[${i}][quantity]" value="${item.quantity}" onchange="invoiceItems[${i}].quantity=parseFloat(this.value)||0;render()" required></td>
                    <td><input type="number" min="0" step="0.01" name="items[${i}][unit_price]" value="${item.unit_price}" onchange="invoiceItems[${i}].unit_price=parseFloat(this.value)||0;render()" required></td>
                    <td>${sym()}${fmt(total)}</td>
        ...
        function recalculate(subtotal) {
            if (subtotal === undefined) subtotal = parseFloat(document.getElementById('subtotal-hidden').value) || 0;
            const tax = parseFloat(document.getElementById('tax').value) || 0;
            const paid = parseFloat(document.getElementById('paid').value) || 0;
            document.getElementById('remaining-display').textContent = sym() + fmt((subtotal + tax) - paid);
        }

        function addRow() { invoiceItems.push({ name: '', quantity: 1, unit_price: 0, unit_type: 'single', unit_detail: '' }); render(); }
        function deleteRow(i) { invoiceItems.splice(i, 1); if (invoiceItems.length === 0) addRow(); else render(); }
        function updateCurrencySymbol() { currentCurrency = document.getElementById('currency').value; render(); }
    </script>
</body>
</html>
